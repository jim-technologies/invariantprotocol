...
async def test_connect_greet(server):
...
async def test_connect_greet_with_enum_and_tags(server):
...
async def test_connect_greet_group(server):
...
async def _run_remote_mcp_session(grpc_port: int, messages: list[str]) -> list[dict]:
    ...
    Runs subprocess in a thread so the test loop stays free to serve the
    gRPC backend the subprocess proxies to.
    """
    stdin_data = "\n".join(messages) + "\n"
...
async def test_remote_mcp_tools_list(server):
...
async def test_remote_mcp_tool_call(server):
...
async def test_remote_mcp_tool_call_with_enum(server):
